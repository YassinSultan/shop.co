...
export default function Wishlist() {
  const { removeFromWishlist, addToWishlistLoading, wishlist, addAllToCart } =
    useWishlist();
  ...
  return (
    <>
      <section className="container py-4 flex flex-col justify-center items-center">
        <div className="w-full flex  justify-between items-center">
          <h2 className="text-4xl font-extrabold font-title">
            Wishlist <span>({wishlist.data.length})</span>
          </h2>
          {/* add all products */}
          <button
            className="btn-primary"
            onClick={() => {
              addAllToCart({ productsArray: wishlist.data });
            }}
          >
            Add All to Cart
          </button>
